...
wiced_bt_gatt_status_t thermistor_connect_callback(wiced_bt_gatt_connection_status_t *p_conn_status)
{
    ...
    if ((p_conn_status->connected) && (thermistor_conn_id == 0))
    {
        ...
        {
            WICED_BT_TRACE("\r\nUnable to configure preferred PHY as 2M PHY\r\n");
        }
    }
    else
    {
        ...
        /* Reset the CCCD value so that on a reconnect CCCD (notifications) will be off */
...
/*
 ...
 Function Description:
 @brief  The function is invoked when GATTS_REQ_TYPE_WRITE is received from the client device and is invoked
         by GATT Server Event Callback function. This handles "Write Requests" received from Client device.
                                                ...
                                                uint16_t conn_id)
{
    ...
    if (p_write_req->handle > HANDLE_OTA_FW_UPGRADE_SERVICE)
    {
...
/*
 ...
 Function Description:
 @brief  The function is invoked when GATTS_REQ_TYPE_CONF is received from the client device and is invoked by GATT Server Event Callback function. This handles "Read Requests" received from Client device
...
/*
 ...
 Function Description:
 @brief  The function is invoked by thermistor_connect_callback when the device is connected.

 @param p_conn_status   wiced_bt_gatt_connection_status_t to set the phy preferences for specific BD_ADDR
