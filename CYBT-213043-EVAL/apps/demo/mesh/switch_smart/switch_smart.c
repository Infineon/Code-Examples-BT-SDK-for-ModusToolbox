...
uint8_t mesh_appearance[WICED_BT_MESH_PROPERTY_LEN_DEVICE_APPEARANCE]               = { BIT16_TO_8(APPEARANCE_SENSOR_MOTION) };
...
#define MESH_ONOFF_SWITCH_ELEMENT_INDEX     0
...
#include "fid_app.h"
    ...
    wiced_bt_mesh_core_set_trace_level((1 << FID_MESH_APP__MESH_UTIL_C), 0);
#endif
...
#if 0
    // As we do not know if we are starting up or returning from hid off, initialize the sensor
    // with shortest blind time, so that we receive interrupt as soon as possible.  In the first
    // interrupt we will adjust blind time so that sensor does not interrupt us too often
    e93196_usr_cfg.e93196_init_reg.blind_time = 1;
#endif
...
void mesh_onoff_client_message_handler(uint16_t event, wiced_bt_mesh_event_t *p_event, void *p_data)
{
    ...
    if (event == WICED_BT_MESH_ONOFF_STATUS)
    {
    }
