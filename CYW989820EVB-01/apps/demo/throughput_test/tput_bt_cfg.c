 ...
 ****************************************************************************/
const wiced_bt_cfg_settings_t tput_bt_cfg_settings =
    {
        .device_name               = (uint8_t*)"TPUT",/**< Local device name (NULL terminated) */
        ...
        .gatt_cfg =                       /**< GATT settings */
            {
                ...
                .server_max_links = 1, //0,   /**< Server config: maximum number of remote clients connections allowed by the local */
                .max_attr_len     = 512, //0,   /**< Maximum attribute length; gki_cfg must have a corresponding buffer pool that can hold this length */
                .max_mtu_size     = 527 //256
            },
 ...
 *****************************************************************************/
const wiced_bt_cfg_buf_pool_t tput_bt_cfg_buf_pools[WICED_BT_CFG_NUM_BUF_POOLS] =
    {
            { 64,       24 },     /* Small Buffer Pool */
            { 360,      16 },     /* Medium Buffer Pool (used for HCI & RFCOMM control messages, min recommended size is 360) */
            { 720,      10 },     /* Large Buffer Pool  (used for HCI ACL messages) */
            { 1024,     10 },     /* Extra Large Buffer Pool - Used for avdt media packets and miscellaneous (if not needed, set buf_count to 0) */
    };
