...
extern tRemoteAppState  *bleRemoteAppState;
extern wiced_timer_t allow_sleep_timer;
#endif
 ...
 * */
wiced_bt_gatt_status_t bleremote_gatts_req_read_handler( uint16_t conn_id, wiced_bt_gatt_read_t * p_read_data )
{
 ...
 * */
wiced_bt_gatt_status_t bleremote_gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_write_t * p_data )
{
...
#ifdef OTA_SECURE_FIRMWARE_UPGRADE
            if (wiced_ota_fw_upgrade_init(&ecdsa256_public_key, bleremoteapp_ota_fw_upgrade_status, NULL) == WICED_FALSE)
#else
            if (wiced_ota_fw_upgrade_init(NULL, bleremoteapp_ota_fw_upgrade_status, NULL) == WICED_FALSE)
#endif
            {
...
wiced_bt_gatt_status_t bleremote_gatts_connection_up( wiced_bt_gatt_connection_status_t *p_status )
{
    WICED_BT_TRACE( "bleremote_conn_up, id: %d, peer_addr_type: %d, peer_addr: %B, second_conn_state: %d\n",  p_status->conn_id, p_status->addr_type, p_status->bd_addr, ble_hidd_link.second_conn_state);
 ...
 * */
wiced_bt_gatt_status_t bleremote_gatts_connection_down( wiced_bt_gatt_connection_status_t *p_status )
{
...
wiced_bt_gatt_status_t bleremote_gatts_conn_status_cb( wiced_bt_gatt_connection_status_t *p_status )
{
    if(p_status->connected)
    {
        return bleremote_gatts_connection_up( p_status );
    }
    else
    {
        return bleremote_gatts_connection_down( p_status );
    }
}
 ...
 */
wiced_bt_gatt_status_t bleremote_gatts_req_conf_handler( uint16_t conn_id, uint16_t handle )
{
    WICED_BT_TRACE( "bleremote_gatts_req_conf_handler, conn %d hdl %d\n", conn_id, handle );
...
wiced_bt_gatt_status_t bleremote_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_data )
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_SUCCESS;

    //WICED_BT_TRACE( "_bleremote_gatts_req_cb. conn %d, type %d\n", p_data->conn_id, p_data->request_type );
    switch ( p_data->request_type )
    {
        case GATTS_REQ_TYPE_READ:
            result = bleremote_gatts_req_read_handler(p_data->conn_id, &(p_data->data.read_req));
            break;

        case GATTS_REQ_TYPE_WRITE:
        case GATTS_REQ_TYPE_PREP_WRITE:
            result = bleremote_gatts_req_write_handler(p_data->conn_id, &(p_data->data.write_req));
             break;
       ...
       case GATTS_REQ_TYPE_CONF:
           result = bleremote_gatts_req_conf_handler( p_data->conn_id, p_data->data.handle );
           break;

       default:
            WICED_BT_TRACE("Please check this bleremote_gatts_req_cb!!!\n");
            break;
    }
...
wiced_bt_gatt_status_t bleremote_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_SUCCESS;

    //WICED_BT_TRACE("\nbleremote_gatts_callback event: 0x%x", event);

    switch(event)
    {
        case GATT_CONNECTION_STATUS_EVT:
            result = bleremote_gatts_conn_status_cb(&p_data->connection_status);
            break;

        case GATT_ATTRIBUTE_REQUEST_EVT:
            result = bleremote_gatts_req_cb(&p_data->attribute_request);
            break;
#if 0
        case 5: //GATT_CONGESTION_EVT:
